...
def restore_model(env: Dict[str, str], model_class: Callable, kwargs: Dict[str, any]) -> torch.nn.Module:
    ...
    weights = torch.load(env['model_weights_path'], map_location=device)
    if 'extractor' in weights:
        model.load_state_dict(weights['extractor'])
    else:
...
def get_data(env: Dict[str, str], train_ratio: float = .85, valid_ratio: float = .1, cached: bool = False) -> \
        (DataLoader, Dataset):
    tqdm.write("Loading dataset")
    if not cached:
        data = DatasetV2.from_path(env['dataset_dir'], logrank_b=10)
    else:
    ...
    data_loader = DataLoader(data, 1, shuffle=False, num_workers=0, collate_fn=lambda b: b)
