...
\begin{itemize}
	\item Detection based on calculation of the deviation of \textit{change} matrices. 
	\item Detection based on \texttt{OnLoadEnd()}-method of \texttt{CEF 3} and hard coded waiting time.
\end{itemize}

Our algorithm based on calculation of the deviation of change matrices, assumes that the number of changed pixels per screenshot will dramatically reduce over time. In other words, given the screenshots $I_{t}, I_{t+1}, I_{t+2}\in \{0, ... , 255\}^{width\times height \times 4}$ taken at times $t$, $t+1$,  and $t+2$ respectively, the total number of changed pixels  $|\Delta_{t,t+1}|$ between $I_{t}$ and $I_{t+1}$ will be less than $|\Delta_{t+1,t+2}|$. Furthermore, we assume that the $|\Delta|$ will converge for $t \rightarrow \infty$ meaning that the website has finished loading.
...
We compute the total number of changes from $I_{t}$ to $I_{t+1}$ with matrix norm $|| C_{t,t+1}||$.
...
coordinates {
	(0,1500000)(10,750000)(20,325000)(30,162500)(40,81250)(50,40000)
};
\end{axis}
