...
In general all browsers have similar steps to render a website: They download the source code of the requested webpage, parse and build the DOM. The DOM is build and represented as a tree, where nodes are \texttt{HTML}-tags and children of a node represent nested \texttt{HTML}-tags. In \texttt{CEF 3}, we are able to directly access the DOM after a website has finished rendering. The \texttt{isLoading}-parameter of \texttt{OnLoadingStateChange()}-method in \texttt{CefLoadHandler} is used for detection. It is true if the given website has finished rendering and false if not. Furthermore the DOM can be accessed by implementing \texttt{Visit()}-method of \texttt{CefDOMVisitor}. While \texttt{CefLoadHandler} runs in the browser-process, \texttt{CefDOMVisitor} has to run in the render-process, due to the nature of DOM being build in the render-process. Leading to the problem that \texttt{CefDOMVisitor} can only be created and run in the render-process. Therefore, we implement \texttt{OnMessageReceived()} of \texttt{CefRenderProcessHandler} to create an instance of \texttt{CefDOMVisitor}, which runs in the render-process. In \texttt{CefRenderProcessHandler} the default behaviour of the render-process can be extended or overriden. \texttt{OnMessageReceived()}-method is invoked if the render-process receives any IPC-message from any other process. 

Having all important components together, this leads to the following workflow of the URL-Datamodule:

\begin{enumerate}
	\item Return in \texttt{CefApp} our custom implementation of \texttt{CefRenderProcessHandler} and initialize \texttt{CEF 3}.
	\item Create and start a browser with custom \texttt{CefClient} implementation returning custom \texttt{CefLoadHandler} implementation.
